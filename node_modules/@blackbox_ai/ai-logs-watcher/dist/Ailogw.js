...
// src/Ailogw.ts
import { AilogwException, IncompleteConfig } from "./exceptions/AilogwException.js";
import twilio from "twilio";
import { AilogwVoiceProvider, ElevenLabsVoiceProvider } from "./voice/index.js";
import { appendFileSync } from "fs";
import { OpenAI } from "openai";
var lineMarkerRegex = /^__LINE_\d+__$/;
var LogDiagnosticSchema = zod_default.object({
  status: zod_default.enum(["critical", "warning", "normal"]).describe("Indicates the overall health status of the logs. Can be 'critical', 'warning', or 'normal'."),
  topErrors: zod_default.array(zod_default.string().regex(lineMarkerRegex)).max(3).optional().nullable().describe("Up to three line markers (an array index may not contain multiple line markers) (e.g., __LINE_2__) of the most critical log entries, corresponding to the __LINE_<number>__ markers in the provided logs. Only present if status is 'critical'.")
...
var AILOGW_TWILIO_ACCOUNT_SID = "AILOGW_TWILIO_ACCOUNT_SID";
var AILOGW_TWILIO_AUTH_TOKEN = "AILOGW_TWILIO_AUTH_TOKEN";
var AILOGW_TWILIO_NUMBER_FROM = "AILOGW_TWILIO_NUMBER_FROM";
var ELEVENLABS_API_KEY = "ELEVENLABS_API_KEY";
var BLACKBOX_API_MODEL = "BLACKBOX_API_MODEL";
var BLACKBOX_API_KEY = "BLACKBOX_API_KEY";
var getAilogwDefaultOptions = () => ({
  log: false,
  activate: true,
  twilio: {
  ...
  modelSettings: {
    model: process.env[BLACKBOX_API_MODEL] != void 0 ? process.env[BLACKBOX_API_MODEL] : "blackboxai/openai/gpt-4.1-mini"
  },
  polling: {
    delay: process.env["AILOGW_POLL_DELAY"] != void 0 ? process.env["AILOGW_POLL_DELAY"] : "10:minutes",
    tailAmount: 10
  }
});
var AilogwDefaultOptions = getAilogwDefaultOptions();
var Ailogw = class {
  #options;
  #status = {
    polling: "paused",
  ...
  #logs = [];
  #intervalId = -1;
  #internalLog = process.env["AILOGW_LOG"] != void 0 && process.env["AILOGW_LOG"] == "true";
  #twilio;
  #voiceProvider;
   ...
   */
  constructor(options) {
    const defaultOptions = getAilogwDefaultOptions();
    const gettwilioOptions = () => {
      const accountSid = options.twilio?.accountSid ?? defaultOptions.twilio.accountSid;
      const authToken = options.twilio?.authToken ?? defaultOptions.twilio.authToken;
      const numberFrom = options.twilio?.numberFrom ?? defaultOptions.twilio.numberFrom;
      if (!accountSid)
    ...
    this.#options = {
      ...
      activate: options.activate ?? defaultOptions.activate,
      clientOptions: options.clientOptions ?? defaultOptions.clientOptions,
      modelSettings: options.modelSettings ?? defaultOptions.modelSettings,
      polling: options.polling ? { ...options.polling, tailAmount: options.polling.tailAmount ?? defaultOptions.polling.tailAmount } : defaultOptions.polling,
      events: options.events
    };
    this.#hasModelSettings = (() => {
      const { model, ...rest } = this.#options.modelSettings;
    ...
    const customClient = new OpenAI(this.options.clientOptions);
  ...
  createAlertDiagnostic(params, logs) {
    ...
    if (params.topErrors && params.topErrors.length > 0) {
      const errorLines = params.topErrors.map((marker) => {
        const match = marker.match(/^__LINE_(\d+)__$/);
        if (match && match[1]) {
          const lineNumber = parseInt(match[1], 10);
          const idx = lineNumber - 1;
          return logs[idx] !== void 0 ? `${lineNumber}| ${logs[idx]}` : `${lineNumber}: (log not found)`;
        }
        return marker;
      }).join("\n");
      formatted += `
...
${formatted}`
  ...
  createLogger(options) {
    ...
    return {
      ...originalConsole,
      log: (...args) => {
        const complete = [(/* @__PURE__ */ new Date()).toISOString(), ...args];
        this.feedLog(complete.join(" "));
        originalConsole.log(...complete);
        if (options?.destination?.log) {
          try {
            appendFileSync(options.destination.log, complete.join(" ") + "\n");
          } catch (error) {
            originalConsole.error("Failed to write log to file:", error);
          }
        }
      },
      error: (...args) => {
        const complete = [(/* @__PURE__ */ new Date()).toISOString(), "ERROR", ...args];
        this.feedLog(complete.join(" "));
        originalConsole.error(...complete);
        if (options?.destination?.error) {
          try {
            appendFileSync(options.destination.error, complete.join(" ") + "\n");
          } catch (error) {
            originalConsole.error("Failed to write error log to file:", error);
          }
        }
      },
      warn: (...args) => {
        const complete = [(/* @__PURE__ */ new Date()).toISOString(), "WARN", ...args];
        this.feedLog(complete.join(" "));
        originalConsole.warn(...complete);
        if (options?.destination?.warn) {
          try {
            appendFileSync(options.destination.warn, complete.join(" ") + "\n");
          } catch (error) {
            originalConsole.error("Failed to write warn log to file:", error);
          }
        }
      },
      info: (...args) => {
        const complete = [(/* @__PURE__ */ new Date()).toISOString(), "INFO", ...args];
        this.feedLog(complete.join(" "));
        originalConsole.info(...complete);
        if (options?.destination?.info) {
          try {
            appendFileSync(options.destination.info, complete.join(" ") + "\n");
          } catch (error) {
            originalConsole.error("Failed to write info log to file:", error);
          }
        }
      },
      debug: (...args) => {
        const complete = [(/* @__PURE__ */ new Date()).toISOString(), "DEBUG", ...args];
        this.feedLog(complete.join(" "));
        originalConsole.debug(...complete);
        if (options?.destination?.debug) {
          try {
            appendFileSync(options.destination.debug, complete.join(" ") + "\n");
          } catch (error) {
            originalConsole.error("Failed to write debug log to file:", error);
          }
        }
      }
    };
  ...
  createPollingRoutine() {
    return async () => {
      ...
      let readFromLast = true;
      let logsToAnalyze;
      if (this.#options.polling.timestampConfig) {
        readFromLast = false;
        const { regex, parser, timeWindow } = this.#options.polling.timestampConfig;
        const timeWindowMs = this.convertDelayToMs(timeWindow);
        const now = /* @__PURE__ */ new Date();
        const fromTime = new Date(now.getTime() - timeWindowMs);
        logsToAnalyze = this.#logs.filter((log) => {
          ...
          let logDate;
          try {
            if (parser === "iso" || !parser) {
              logDate = new Date(timestampStr);
            } else {
              logDate = parser(timestampStr);
            }
            if (isNaN(logDate.getTime())) {
              return false;
            }
            return logDate >= fromTime && logDate <= now;
          } catch (error) {
            return false;
          }
        });
        if (logsToAnalyze.length === 0) {
          if (this.#internalLog)
            this.log("info", `No logs found in time window (last ${timeWindow})`);
          return;
        }
        this.log("info", `Analyzing ${logsToAnalyze.length} logs from time window (last ${timeWindow})`);
      } else {
        ...
        logsToAnalyze = this.#logs.slice(this.#status.lastPollingLogIndex, this.#status.lastPollingLogIndex + this.#options.polling.tailAmount);
        this.#status["lastPollingLogIndex"] = this.#status.lastPollingLogIndex + logsToAnalyze.length;
      }
      const formattedLogs = logsToAnalyze.map((log, index) => `__LINE_${index + 1}__:${log}`).join("\n");
      const provideDiagnosticTool = tool({
        name: "provide_diagnostic",
        description: "provide your final and complete diagnostic with the informations provided and following the instructions",
        parameters: LogDiagnosticSchema,
        execute: async (params) => {
          this.#options.events.alert({
            options: this.#options,
            logs: this.#logs,
            diagnostic: this.createAlertDiagnostic(params, logsToAnalyze),
      ...
      try {
        const watcherAgent = new Agent({
          name: "Log watcher",
          tools: [provideDiagnosticTool],
          instructions: `Your task is to analyze the user provided ${logsToAnalyze.length} ${readFromLast ? "last " : ""}lines of logs and extract relevant data.
                The logs are formatted as __LINE_<line_number>__:<log_content> where each line is prefixed with a unique line marker.
                ...
                Severity weighting and status selection:
                - Log levels do not have the same weight: critical > warning > normal.
                - Determine the overall 'status' using the highest-weight level present in the provided lines.
                - If any critical logs are present, status must be "critical" (no exceptions). If there are no critical logs but at least one warning, status must be "warning". Otherwise, set status to "normal".
                - Only include 'topErrors' when status is "critical", and the entries must be the exact __LINE_<number>__ markers (max 3) of the most critical lines.
                
                Keyword heuristics for critical detection:
                - Treat a line as critical if it contains terms strongly associated with outages or catastrophic failure, even if the level isn't explicitly "CRITICAL".
                - Examples include (non-exhaustive): FATAL, PANIC, EMERGENCY, SEVERE, ALERT, OOM, OUT OF MEMORY, SEGFAULT, SIGSEGV, CORE DUMP/CORE DUMPED, UNHANDLED EXCEPTION, CRASH, KERNEL PANIC, DATA LOSS, DATA CORRUPTION/CORRUPTED, UNRECOVERABLE, SECURITY BREACH/COMPROMISE, DISK FAILURE, RAID DEGRADED, SERVICE UNAVAILABLE/SERVICE DOWN, DEADLOCK, PARTITION LOST.
                - When unsure between warning and critical, prefer "critical" (a false positive is better than a false negative).
                - If you up-rank a line to critical based on these keywords, include it in 'topErrors' (subject to the max of 3).
                
                If any critical logs are found, you must provide the line numbers from the __LINE_<number>__ markers of up to 3 critical log entries.
                The line numbers should correspond exactly to the numbers shown in the __LINE_<number>__ markers in the provided logs.
                
                You MUST use the provide_diagnostic tool to return your analysis.`,
          ...this.#options.modelSettings
        });
        await run(watcherAgent, `Provide your diagnostic for the following logs:
${formattedLogs}`);
      } catch (e) {
        console.error(e);
      }
  ...
  async triggerAnalysis(options) {
    ...
    if (this.#logs.length === 0) {
      this.log("info", "No logs to analyze");
      return;
    }
    let logsToAnalyze;
    if (options?.timeRange) {
      const { from, to, timestampConfig } = options.timeRange;
      logsToAnalyze = this.#logs.filter((log) => {
        const match = log.match(timestampConfig.regex);
        if (!match || !match[1]) {
          return false;
        }
        const timestampStr = match[1];
        let logDate;
        try {
          if (timestampConfig.parser === "iso" || !timestampConfig.parser) {
            logDate = new Date(timestampStr);
          } else {
            logDate = timestampConfig.parser(timestampStr);
          }
          if (isNaN(logDate.getTime())) {
            this.log("warn", `Invalid timestamp parsed from log: ${timestampStr}`);
            return false;
          }
          return logDate >= from && logDate <= to;
        } catch (error) {
          this.log("warn", `Error parsing timestamp from log: ${timestampStr}`, error);
          return false;
        }
      });
      if (logsToAnalyze.length === 0) {
        this.log("info", `No logs found in time range ${from.toISOString()} to ${to.toISOString()}`);
        return;
      }
      this.log("info", `Found ${logsToAnalyze.length} logs in time range ${from.toISOString()} to ${to.toISOString()}`);
    } else {
      const tailAmount = options?.tailAmount ?? this.#options.polling.tailAmount;
      const startIndex = tailAmount ? Math.max(0, this.#logs.length - tailAmount) : 0;
      logsToAnalyze = this.#logs.slice(startIndex);
      if (logsToAnalyze.length === 0) {
        this.log("info", "No logs to analyze");
        return;
      }
    ...
    const provideDiagnosticTool = tool({
      ...
      parameters: LogDiagnosticSchema,
      execute: async (params) => {
        await this.#options.events.alert({
          options: this.#options,
          logs: this.#logs,
          diagnostic: this.createAlertDiagnostic(params, logsToAnalyze),
          sendSms: this.sendSms.bind(this),
    ...
    try {
      const watcherAgent = new Agent({
        name: "Log watcher",
        tools: [provideDiagnosticTool],
        instructions: `Your task is to analyze the user provided ${logsToAnalyze.length} lines of logs and extract relevant data.
                The logs are formatted as __LINE_<line_number>__:<log_content> where each line is prefixed with a unique line marker.
...
${formattedLogs}`);
      if (flush) {
        this.#logs = [];
        this.#status.lastPollingLogIndex = 0;
        this.log("info", "Logs flushed after analysis");
      }
    } catch (e) {
      console.error("Error during triggered analysis:", e);
      throw e;
    }
  }
  /**
...
export {
  ...
  AILOGW_TWILIO_NUMBER_FROM,
  Ailogw,
  AilogwDefaultOptions,
  BLACKBOX_API_KEY,
  BLACKBOX_API_MODEL,
  ELEVENLABS_API_KEY,
  LogDiagnosticSchema
