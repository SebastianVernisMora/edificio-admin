// Gestión de Cierres Contables - Admin Panel
class CierresManager {
  ...
  async handleCierreAnualSubmit() {
    try {
      ...
      // Mostrar mensaje de éxito con información de cuotas generadas
      let mensaje = `Cierre anual de ${cierreData.año} generado correctamente`;
      
      if (result.cierre && result.cierre.cuotasSiguienteAño) {
        mensaje += `. ${result.cierre.cuotasSiguienteAño.mensaje}`;
      }
      
      this.showSuccess(mensaje, 8000); // Mostrar por más tiempo
  ...
  async viewCierre(cierreId) {
    try {
      ...
      // Información de cuotas generadas (solo para cierres anuales)
      if (cierre.tipo === 'anual' && (cierre.cuotasSiguienteAño || cierre.detalles?.cuotasSiguienteAño)) {
        const cuotasInfo = cierre.cuotasSiguienteAño || cierre.detalles.cuotasSiguienteAño;
        html += `
          <div class="row mb-4">
            <div class="col-12">
              <div class="card border-0 shadow-sm">
                <div class="card-header bg-white">
                  <h5 class="card-title mb-0">
                    ...
                    Cuotas Año Siguiente
                  </h5>
                </div>
                <div class="card-body">
                  <div class="row align-items-center">
                    <div class="col-md-8">
                      <h6 class="text-muted mb-1">Año ${cuotasInfo.año}</h6>
                      <p class="mb-0">${cuotasInfo.mensaje}</p>
                    </div>
                  ...
                  ${cuotasInfo.generadas ? 
                    '<div class="alert alert-success mt-3 mb-0"><i class="bi bi-info-circle me-2"></i>Las cuotas mensuales fueron generadas automáticamente con vencimiento al final de cada mes correspondiente.</div>' : 
                    '<div class="alert alert-warning mt-3 mb-0"><i class="bi bi-exclamation-triangle me-2"></i>Las cuotas no pudieron ser generadas automáticamente.</div>'
                  }
                </div>
              </div>
            </div>
  ...
  async exportCierre(cierreId) {
    try {
      ...
      a.href = url;
      a.download = `cierre_${cierreId}.pdf`;
      ...
      // Mostrar mensaje de éxito
      this.showSuccess('Cierre exportado correctamente');
    } catch (error) {
...
// Make CierresManager globally available for main-controller.js
window.CierresManager = CierresManager;
